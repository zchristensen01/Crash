...
| `parameters.py` | **The research record.** 128 parameters, each with a range, a confidence level and a citation. The authority for every number. Anything no rule reads must be listed in `DEFERRED` with a reason, and a test enforces that both ways. |
...
| `test/` | 145 tests, `node --test`, no framework. Fourteen are `todo` by design: they record where the model disagrees with a published estimate, or a defect that a later phase of the current audit is committed to fixing. A `todo` prints its measured number on every run, so a disagreement cannot go quiet. |
