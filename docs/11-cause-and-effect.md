<!-- cause-effect-fingerprint: f1a8588676b42adf (1484 numbers) -->
# 11 — Cause and Effect
...
#### The decision it contains

Each policy applied before the first month and held, against the same
`debt_trap` opening. Same convention as the preset paths above — events off,
endings on, 96 months — so the rows are comparable with them.
...
**You cannot consolidate your way out, and you cannot cheapen your way out
either.** Austerity alone buys thirteen months and still loses. Cutting the
cost of the debt alone buys twenty-two and still loses. **Only the
combinations survive**, and the second one is a real decision with a real
price: the economy that lives is running a −5.9 output gap eight years later
with debt still at 184% — you do not get out of a debt trap, you get to stay
in it.

> **THIS TABLE HAD NO PRODUCER UNTIL 5.21, AND FOUR OF ITS FIVE ROWS WERE
> WRONG.** It was measured by hand for `docs/12`, pasted here, and flagged as
> not re-run — and the flag understated it, saying only the do-nothing row had
> moved. `rate to the floor` read ***survives*, at 5.1% inflation and a +6.8
> gap**; it reaches a debt crisis in month 95, and the sentence under the table
> used to draw the lesson *"cutting the cost of the debt alone works"* from it.
> The headline lesson is unharmed and is now stronger, which is why this was a
> document defect and not a model finding: the conclusion was right and every
> number under it had drifted. It is generated by
> `node tools/cause-effect.mjs policy` and checked cell by cell like the rest.
